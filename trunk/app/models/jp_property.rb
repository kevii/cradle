class JpProperty < ActiveRecord::Base
  # mysql table used
  self.table_name = "jp_properties"
  ...
  def definition
    if ["pos", "ctype", "cform", "tagging_state", "dictionary", "sth_tagging_state"].include?(self.property_string)
      return nil
    else
      return JpNewProperty.find(:first, :conditions=>["property_string = ?", self.property_string])
    end
  end
  ...
  def self.save_property_tree(type="", sections=[], seperator=nil)
    ...
    0.upto(sections.size-1){|index|
      if index == 0
        item = JpProperty.find(:first, :conditions=>["property_string=? and value=? and parent_id is null", type, sections[index]])
      else
        item = JpProperty.find(:first, :conditions=>["property_string=? and value=? and parent_id=?", type, sections[index], parent_item_id])
      end
      if item.blank?
        if index == sections.size-1
          type_max_id = JpProperty.maximum("property_cat_id", :conditions=>["property_string=?",type])
    ...
    0.upto(sections.size-1){|index|
      if index == 0
        if state.blank? ## ordinary usage
          parent = JpProperty.find(:first, :conditions=>["property_string=? and value=? and parent_id is null", type, sections[index]])
        else  ## for validation of whether null parent_id and 0 property_cat_id item exist or not?
          parent = JpProperty.find(:first, :conditions=>["property_string=? and value=? and parent_id is null and property_cat_id > 0", type, sections[index]])
        end
