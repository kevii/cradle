class JpController < ApplicationController
  ...
  def list
    ...
    if params[:dynamic_lexeme_condition].blank? and params[:dynamic_synthetic_condition].blank?
      @jplexemes = JpLexeme.paginate(:select=>"DISTINCT jp_lexemes.*", :conditions=>params[:static_condition],
      ...
      if params[:static_condition].blank?
        ...
        static_ids = JpLexeme.find(:all, :select=>"DISTINCT jp_lexemes.id", :conditions=>params[:static_condition],
                        				   :joins=>" left join jp_synthetics on jp_synthetics.sth_ref_id = jp_lexemes.id ",
