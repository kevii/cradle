  ...
  has_many :other_properties,  :class_name=>"CnSyntheticNewPropertyItem", :foreign_key=>"ref_id", :dependent => :destroy
	...
	def self.load_struct_from_json(options, current_level = 0, current_max_level = 0)
				...
				if (item.keys.size == 1) && (item.keys[0] =~ /^\d+$/)
					...
					properties_ary = item.inject([]) do |temp_ary, inner|
						unless inner[1].blank?
							property = CnNewProperty.find_by_section_and_property_string('synthetic', inner[0])
							temp_value = case property.type_field
								when 'category' then CnProperty.find_item_by_tree_string_or_array(inner[0], (inner[1] || '')).property_cat_id
		...
		properties_ary.each do |item|
			CnSyntheticNewPropertyItem.create!(
